...
public class TSPGraphNormal  extends TSPSolver
{
        ...
        }else
        {
            ...
            for(int i = 0; i < m_game.getWaypoints().size(); ++i)
            {
                ...
                if(!a_currentPath.includes(i))
                {
                    ...
                    {
                        //search!
                        TSPPath nextPath = new TSPPath(a_currentPath, i, newCost, m_tspBestPath.m_nNodes);
                        _search(nextPath);
                    }
                }
            }
    ...
    private class TSPPath
    {
        ...
        public TSPPath(TSPPath a_base, int a_newNode, double a_newCost, int nNodes)
        {
            m_path = new int[nNodes];
