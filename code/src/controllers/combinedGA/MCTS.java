...
protected static final int ROLLOUT_DEPTH=10;     //50
 ...
 */
public MacroAction execute(Game a_gameCopy, long a_timeDue, boolean a_throwTreeAway)
{
...
private void advanceGame(Game a_game, MacroAction a_move)
{
    ...
    for(int i = 0; i < a_move.m_repetitions; ++i)
    {
...
private void checkBestRoute(double a_score)
{
    if(a_score > m_bestScoreSoFar)
    {
        ...
        /*System.out.format("New best found %.3f : ", a_score);
        for(MacroAction m : m_bestActions)
...
public ArrayList<MacroAction> getBestActions(int a_min)
{
    MCTSNode currentNode = m_rootNode;
    ArrayList<MacroAction> actions = new ArrayList<MacroAction>();
    boolean stop = false;
    ...
    while(!stop)
    {
        ...
        {
            MCTSNode newNode = bestChild(currentNode, a_min);
            if (newNode == null)
            {
                stop = true;
            }
            ...
            {
                actions.add(actions.size(),newNode.m_lastMove);
                currentNode = newNode;
            }
        }
    }
...
/**
 * Gets the best child of the given node.
 * @param a_node the given node.
 * @return the best child (node).
 */
public MCTSNode bestChild(MCTSNode a_node, int a_min)
{
    if(a_node == null || a_node.m_children == null || a_node.m_children.size() == 0)
        return null;
    ...
    for(int i=0;i<values.length;i++)
    {
        values[i]= a_node.m_children.get(i).m_visits; //a_node.m_children.get(i).m_value;
        if(values[i] < a_min)
            values[i] = -1;
    }
    ...
    System.out.print( maxval(values) + "-" );
...
private MCTSNode getChildUCB1(MCTSNode a_node)
{
    double[] values=new double[a_node.m_children.size()];
    double part=Math.log(a_node.m_visits + 1);
    double minFitness = 0;
...
private Vector2d getVisiblePointInPath(Game a_game,Path a_p)
{
...
private Vector2d getBestTargetPointInPath(Game a_game, Waypoint a_closestW, Path a_p)
{
...
private Path getPathToWaypoint(Game a_game, Waypoint a_closestW)
{
...
public double maxval(double... sequence)
{
    try{
